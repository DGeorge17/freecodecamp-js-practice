# freeCodeCamp JavaScript Practice

This repository contains small JavaScript projects created while learning
programming through freeCodeCamp.

## Practice Projects

### Greeting Bot (Workshop)
- Simple program that greets the user
- Built using JavaScript
- Concepts used:
  - Variables
  - Strings
  - console.log

 ### JavaScript Fun Facts Bot (Lab)
- Small JavaScript practice project from freeCodeCamp
- Concepts used:
  - Variables (`let`)
  - String concatenation
  - Reassigning values
  - Console output

 ### Build a Sentence Maker (Lab)
Description
JavaScript practice project from FreeCodeCamp to build dynamic stories using variables and string concatenation. The second story follows the same logic with new variable values and is printed with console.log.
- Learnings:
  - Declare and reassign variables.
  - Concatenate strings correctly.
  - Ensure spaces and formatting match FreeCodeCamp tests.

 ### Build a Teacher Chatbot (Workshop)
 In this workshop project, I built a Teacher Chatbot using JavaScript that simulates structured responses based on user input. The main focus was practicing core JavaScript fundamentals through a simple chatbot logic.
- Key concepts applied:
  - Variables.
  - Bracket Notation.
  - String, string interpolation and substring.
  - Template literals.
